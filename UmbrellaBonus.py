...
custkey = "<insert-enforcement-key>"

# copy paste API key from investigate within the quotes
APIkey = "<insert-investigate-key>"
...
for domain in domainList:
    ...
    if(reqGet.status_code == 200):
        ...
        output = reqGet.json()
        # handle when there is no associated sample
        if output["samples"] == []:
            continue

        # if there is a sample, grab some metadata
        ...
        if scoreSample >= 90:
            ...
            # create "event" to upload to enforcement block list
            data = {
            "alertTime": time + "Z",
